...
{
    public partial class AddTransporteComponent : ComponentBase
    {
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            transporte = new FindSupermarket.Models.FindSupermarketDb.Transporte(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(FindSupermarket.Models.FindSupermarketDb.Transporte args)
        {
            try
            {
                var findSupermarketDbCreateTransporteResult = await FindSupermarketDb.CreateTransporte(transporte);
                DialogService.Close(transporte);
            }
            catch (System.Exception findSupermarketDbCreateTransporteException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new Transporte!" });
            }
        }
