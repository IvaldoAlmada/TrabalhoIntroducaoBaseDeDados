...
namespace FindSupermarket.Pages
{
    public partial class AddProdutoComponent : ComponentBase
    {
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            produto = new FindSupermarket.Models.FindSupermarketDb.Produto(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(FindSupermarket.Models.FindSupermarketDb.Produto args)
        {
            try
            {
                var findSupermarketDbCreateProdutoResult = await FindSupermarketDb.CreateProduto(produto);
                DialogService.Close(produto);
            }
            catch (System.Exception findSupermarketDbCreateProdutoException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new Produto!" });
            }
        }
